...
var contractAddress = "0x3e63edC253955742E914f716b05e3aeF4d7F22ED";
...
$("#setForm").submit((e) => {
  ...
  contractInstance.methods
    ...
    .on("error", function (_err, receipt) {
...
function loadData(blockNumber) {
  ...
  contractInstance.methods.get().call(blockNumber, (err, _storedData) => {
    if (err) return error(err);
    storedData = _storedData;
    updateWhenDone();
  });

  contractInstance.methods.getOwner().call(blockNumber, (err, _owner) => {
    if (err) return error(err);
    owner = _owner;
    updateWhenDone();
  });
